...
public:
    ...
    RollbackTestFixture() {
        const std::string data_home = "/tmp/isched-test-rollback-" + g_run_suffix;
        ...
        client = std::make_unique<isched::test::GraphQLTestClient>("localhost", 18085);
    }
    ...
    const isched::test::AuthPayload& ensure_auth() {
        if (!auth.has_value()) {
            const std::string email = "rollback-admin-" + g_run_suffix + "@example.com";
            auth = client->bootstrap_or_login_platform_admin(email, "RollbackAdminPass!123", "Rollback Admin");
        }
        return auth.value();
    }
